        ...
        {data.items.map((it, idx) => (
          <AnimeLink key={it.url + idx} seriesUrl={it.url} postId={it.postId} className="group block">
            <div className="aspect-[2/3] overflow-hidden rounded-md bg-white/5 border border-white/10">
              {it.image ? (
                <Image unoptimized className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" src={it.image.startsWith('data:') ? it.image : `${getApiBaseUrl()}/api/image?src=${encodeURIComponent(it.image)}`} alt={it.title || "Anime"} width={300} height={450} />
              ) : (
